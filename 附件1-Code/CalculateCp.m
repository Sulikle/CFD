...
feflo_bump = fopen('DGP1_data.dat', 'rb');%open the file
for i = 1:2
    line=fgetl(feflo_bump);
end
...
gamma=1.4;Ma_far=1.1;afa_attack=0;
...
if COORD(1,ip1)>=1&&COORD(1,ip1)<=2&&COORD(2,ip1)>=0&&COORD(2,ip1)<1
    ...
    Cp(3,ip1)=(P_point(1,ip1)-P_far)/(0.5*rho_far*(u_far^2+v_far^2));
end
if COORD(1,ip2)>=1&&COORD(1,ip2)<=2&&COORD(2,ip2)>=0&&COORD(2,ip2)<1
    Cp(1,ip2)=COORD(1,ip2);
    Cp(2,ip2)=COORD(2,ip2);
    Cp(3,ip2)=(P_point(1,ip2)-P_far)/(0.5*rho_far*(u_far^2+v_far^2));
end
    end
end
...
ylabel('Coefficient P','FontSize',15);     
grid on
hold off
